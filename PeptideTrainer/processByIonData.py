...
csvPath = [fileLocation + 'VACVpeps-rackA_peptides.csv'] # fileLocation + 'VACVpeps-rackB_peptides.csv']
print(csvPath)
mgfPath = [fileLocation + 'VACVpeps-rackA.mgf'] # fileLocation + 'VACVpeps-rackB.mgf']
...
def readData(mgfList, csvList):
    ...
    """
    :param mgfList: List of mgfPaths
    :param csvList: List of csvPaths
    :return: BYION Dict in form of {Peptide: [m/z array, intensity array, pepmass]}

    Done via matching scan numbers from csv to mgf and merging
    """
    scanNumbers = {}
    byIonDict = {}
        ...
        with open(csvIter) as csv_file:
            ...
            for row in csv_reader:
                ...
                if row[0][0:2] == row[0][-2:]:
                    continue
                scanNumbers[row[SCAN_CSV_COL]] = row[PEPTIDE_CSV_COL]


        # create a dictionary with the peptide as the key and [[m/z array],[intensity array]] as the value.
        with mgf.read(mgfIter) as mgfReader:
            for spectrum in mgfReader:
                scan = spectrum['params']['scans']
                if scan in scanNumbers.keys():
                    # Should probably change this to a tuple as immutable
                    byIonDict[scanNumbers[scan]] = [spectrum['m/z array'], spectrum['intensity array'],
...
def findByIonIntensities(byIonDict):
    ...
    for peptide, arrays in byIonDict.items():
        ...
        intensitySum = sum(intensityArray)
        ...
        normIntensArray = preprocessing.normalize(intensityArray)
        print(intensityArray)
        print(normIntensArray)
        break
